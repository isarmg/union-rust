...
export interface SunshineHostInfo {
  ...
  reachable: boolean | null; connected: boolean | null;
  connection_error?: string | null;
}
export interface SunshineHostSaveRequest {
  name: string; host: string; web_port: number;
  username: string; password?: string | null; verify_tls: boolean;
}
export type SunshineHostPatchRequest = Partial<SunshineHostSaveRequest>;
export interface SunshineApp {
  name: string; cmd?: string; index: number;
  "image-path"?: string | null; "working-dir"?: string;
  output?: string; "auto-detach"?: boolean; "wait-all"?: boolean;
  "exit-timeout"?: number; "prep-cmd"?: unknown[];
  detached?: unknown[]; elevated?: boolean;
...
export interface MonitoringHostsResponse {
  ...
  /** 库中主机总数。大于 hosts.length 时说明本页之外还有，界面需明示。 */
  total: number;
  limit: number;
  offset: number;
}
