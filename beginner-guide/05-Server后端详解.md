...
- 修改密码序列化执行，先验证再哈希、持久化、发布；接受后的状态转换由独立任务完成，
  并在成功路径用请求进入时捕获的管理员与 request ID 尽力写审计，因此浏览器断开不会留下
  “密码和会话已改变但审计缺失”的取消窗口；
...
“数据库已改、内存未改”的状态。独立任务会在创建前捕获管理员与服务端 request ID，并在
子任务中重新建立审计上下文，因此与业务行同事务提交的审计记录不会退化成 `system` 或丢失关联 ID。

应用保存/关闭/删除、客户端单个或全部解绑与启停、配置保存、PIN 配对、重启、显示重置和
封面上传会改变外部 Sunshine，无法与本地 SQLite 审计组成原子事务。它们会把“完整上游请求
→ 成功后尽力写审计”放进同样携带管理员与 request ID 的独立任务：上游调用返回错误时不写
成功审计；调用返回成功后，即使请求断开也会继续尝试审计；审计数据库失败仅告警，不能把已经
成功的非幂等远端操作伪装成失败并诱发重试。远端系统与本地数据库仍不具备跨系统原子性。
...
公开 ready、需要数据库的管理面路径和 Agent 配对/上报共享同一条数据库边界。文件身份检查
不缓存：每次都在健康缓存前后确认 canonical `unionc.db` 仍是启动时记录的 device/inode、
uid/gid，叶子是权限精确为 0600 的单硬链接普通文件。只读的精确当前 schema 探测才带约
1 秒 TTL；缓存刷新时还会用当前服务身份重新 `O_RDWR` 打开并 `fstat` 同一主库，避免旧文件
描述符在 chmod/chown 后继续工作而掩盖重启故障。连接池在复用旧连接和建立新连接时还通过
SQLite 实际打开的 main 文件句柄核对身份，避免“路径换成 B、打开 B、再把 A 放回”的 ABA
交换。检查发生在连接签出边界，不能撤回已经执行中的事务，因此仍然禁止任何运行中替换。

为什么不能只做 `SELECT 1`？在 Linux 上，即使目录里的数据库被 rename/unlink，SQLite 已打开
的文件描述符仍可能继续读写旧 inode，查询会成功，但重启后打开的已经是缺失文件或另一份库。
一旦进程观察到路径缺失、替换、符号链接、多硬链接、属主或权限变化，数据库身份会被永久
标为失效并持续返回 503；把旧文件或旧权限放回也不能热恢复，必须停服检查并重启。仅有一次
瞬时 reopen I/O 失败且文件元数据没变时，本轮返回不可用，后续探针仍可重试。
