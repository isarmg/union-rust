...
不缓存：每次都在健康缓存前后确认 canonical `unionc.db` 仍是启动时记录的 device/inode，且
叶子是单硬链接普通文件。只读的精确当前 schema 探测才带约 1 秒 TTL；缓存过期时只允许一个
请求执行，其余并发请求等待并复用结果。连接池在复用旧连接和建立新连接时还通过 SQLite
实际打开的 main 文件句柄核对身份，避免“路径换成 B、打开 B、再把 A 放回”的 ABA 交换。
检查发生在连接签出边界，不能撤回已经执行中的事务，因此仍然禁止任何运行中替换。

为什么不能只做 `SELECT 1`？在 Linux 上，即使目录里的数据库被 rename/unlink，SQLite 已打开
的文件描述符仍可能继续读写旧 inode，查询会成功，但重启后打开的已经是缺失文件或另一份库。
一旦进程观察到路径缺失、替换、符号链接或多硬链接，数据库身份会被永久标为失效并持续
返回 503；把旧文件放回也不能热恢复，必须停服检查并重启。
