...
- 主机身份：稳定实例 ID、OS、OS 版本、内核版本、架构、Agent 版本；
...
- 数据目录逐级拒绝符号链接，最终必须由服务 UID 所有且精确 0700；程序不 chmod 未认领的
  既有路径，并用 0600 布局 marker 区分 UnionC 状态目录；
...
- 遥测与审计按独立保留期清理；
- 遥测持续分批至追平（每批 10 000 行）、审计每批 1 000 行独立提交，避免单次长事务独占写锁；
- 每台主机最新报告有保留例外；
...
待消费票据每会话最多 8 张、进程全局最多 256 张，超限返回 429。
...
1. 解析数据目录，逐级校验真实目录并认领/建立私有 0700 布局；
2. 初始化或载入主密钥环；
3. 载入管理员配置；
...
- 默认 10 秒采样、30 秒慢采样、10% 抖动、64 MiB spool（分配块/至少 4 KiB 记账，最多 4096 个文件）；长期断网超过配额时会优先淘汰
...
| 项目 | 当前值/上限 | 含义 |
|---|---|---|
| 管理员会话 | 进程内，绝对上限 7 天；浏览器会话 Cookie | 关闭浏览器或重启 Server 即失效；不是持久身份会话或 SSO |
