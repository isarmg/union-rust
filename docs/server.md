...
数据目录的每一级都必须是真实目录，不能是符号链接；最终目录必须由当前服务 UID 所有且已经是
精确 0700。程序不会为了“修好”一个既有路径而自动 `chmod`，因此误把 `/`、`/tmp`、仓库根或
其他应用目录填入 `UNIONC_DATA_DIR` 时会直接拒绝，不会改变目标权限。只有开发/显式首次
bootstrap 和 `restore` 可以逐级创建缺失目录；其余启动与维护命令要求目录已存在。成功认领后
会持久化 0600 的 `.unionc-data-directory` 布局 marker；它属于运行数据，备份、卸载和重装时
应与目录一起保留。既有非空目录如果没有该 marker，即使属主、权限和文件名看起来正确也会
被拒绝；当前版本不会认领或转换旧数据目录。
...
`unionc.db` 的路径叶子必须是单硬链接普通文件。Server 在 SQLx 打开前通过无符号链接跟随的
文件描述符把主库收紧为 0600，并固定其 device/inode、uid/gid。路径消失、被替换、变成符号
链接、新增硬链接，或主库属主/权限变化后，readiness、依赖数据库的管理面与 Agent 接口都会
fail closed，连接池也不再签出旧连接。约每秒的完整健康探针还会以当前服务身份重新
`O_RDWR` 打开并 `fstat` 同一主库，防止已有 SQLite 描述符掩盖重启时无法打开的问题。首次
观察到确定的身份或元数据异常后，本进程不会因原状态恢复而自动复活；停服检查并重启。
连接检查只能守住后续签出，不能撤回已经执行中的事务，也只覆盖主库而不替代数据目录和
WAL sidecar 的运维检查，所以它不把任何运行中替换方式变成受支持操作。

`tools/dev-server.sh` 会以私有 umask 创建并检查 `.runtime/server`，导出绝对
`UNIONC_DATA_DIR` 后从根 manifest 启动 Server；即使从其他目录调用也不会改变数据位置。
旧脚本曾留下 0755 空目录时，新脚本会拒绝并给出一次性的人工确认/修权命令，不会自行改动
一个尚未证明属于 UnionC 的既有目录。
...
(每批 10 000 行，批间让出 50 毫秒），并持续到本轮启动时确定的超期集合清空。SQLite 同一
时刻只有一个写事务，分批可缩短写锁持有时间，避免日常清理长时间阻塞 Agent 补传。删除会把页放回数据库空闲列表，但不会保证文件
