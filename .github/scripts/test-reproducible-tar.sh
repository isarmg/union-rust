...
mkdir -p "$source_dir/bin" "$source_dir/nested"
printf '#!/bin/sh\nexit 0\n' > "$source_dir/bin/unionc"
chmod 0755 "$source_dir/bin/unionc"
printf 'root payload\n' > "$source_dir/root.txt"
printf 'nested payload\n' > "$source_dir/nested/value.txt"

touch -t 202001020304.05 \
  "$source_dir" "$source_dir/bin" "$source_dir/nested" \
  "$source_dir/bin/unionc" "$source_dir/root.txt" "$source_dir/nested/value.txt"
...
test -x "$work_dir/extracted/bundle/bin/unionc"
