...
mod tests {
    ...
    fn serde_dtos_accept_only_the_current_wire_shape() {
        ...
        assert!(
            serde_json::from_str::<TrayPreferences>(
                r#"{"application_version":"0.3.6","server":"","legacy":true}"#
            )
            .is_err()
        );
        ...
        assert!(
            serde_json::from_str::<PairEvent>(
                r#"{"event":"pairing_waiting","version":"0.3.6","request_id":"request","generation":"generation","activation_url":"https://server.example/agent/activate/request","pairing_endpoint":"https://server.example/api/agent/v2/pairing-requests","expires_at":"2026-08-20T00:00:00Z","poll_interval":2}"#
            )
            .is_ok()
        );
        assert!(
            serde_json::from_str::<PairEvent>(
                r#"{"event":"pairing_waiting","version":"0.3.6","request_id":"request","generation":"generation","activation_url":"https://server.example/agent/activate/request","pairing_endpoint":"https://server.example/api/agent/v2/pairing-requests","poll_interval":2}"#
            )
            .is_err()
        );
        assert!(
            serde_json::from_str::<PairEvent>(
                r#"{"event":"paired","version":"0.3.6","request_id":"request","instance_id":"instance","endpoint":"https://server.example/api/agent/v1/report","legacy":true}"#
            )
            .is_err()
        );
        assert!(
        ...
        assert!(
            serde_json::from_str::<ServerHealthResponse>(
                r#"{"status":"ok","version":"0.3.6","uptime_seconds":1}"#
            )
            .is_ok()
        );
        assert!(
        ...
        assert!(
            serde_json::from_str::<ServerHealthResponse>(
                r#"{"status":"ok","version":"0.3.6","uptime_seconds":1,"legacy":true}"#
            )
            .is_err()
        );
    }
