...
```sh
BINARY=/path/to/unionc-agent VERSION=0.3.3 ./agent/packaging/macos/build-pkg.sh
```
...
VERSION=0.3.3 \
OUTPUT=/path/to/unionc-agent-0.3.3.pkg \
...
```bash
./agent/packaging/macos/tests/smoke-pkg.sh --allow-system-changes dist/unionc-agent-0.3.3.pkg
```

## Install and pair

```sh
sudo installer -pkg unionc-agent-0.3.3.pkg -target /
...
Reinstalling the current 0.3.3 package leaves the old jobs running while Installer replaces only
...
A different package version is not migrated: purge 0.3.3 state before installing another version.
