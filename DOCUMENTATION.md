...
> 2026-08-27 范围更新：Union 已成为五个业务模块的唯一发行单元，目标是编译期选择、
> 运行时独立进程。本文主体仍准确描述当前已实现的主机/Sunshine 单进程代码；新增模块边界
> 与迁移完成条件以 `docs/modules.md` 和 sibling `upstream/REQUIREMENTS-AND-BOUNDARIES.md`
> 为准，未完成的目标不能作为当前功能承诺。
...
数据库可用性不是简单的 `SELECT 1`。Server 在 SQLx 打开前先通过 `O_NOFOLLOW` 文件描述符
把 canonical `unionc.db` 收紧为 0600，再记录 device/inode、uid/gid；每次检查都在缓存前后
确认路径仍指向这个单硬链接普通文件，且属主和私有权限未变。只读的精确当前 schema 比对带
约 1 秒 TTL 的单航缓存，刷新时还以当前服务凭据重新 `O_RDWR` 打开并 `fstat` 同一主库文件。
连接池复用旧连接或建立新连接时同时核对路径身份与 SQLite 实际打开的 main 文件句柄。公开
`/api/ready`、需要库的管理面路径以及 Agent 配对/上报共同使用这条边界。这些检查发生在连接
签出边界，无法撤回已在执行的事务，因此绝不能把它当作支持运行中替换数据库的热切换协议。
一旦运行期观察到路径缺失、替换、符号链接、多硬链接、属主或权限变化，该进程会永久把
数据库标为不可用并返回 503；即使原状态随后恢复，也必须停服检查并重启。单纯的瞬时 reopen
I/O 失败只让本轮探测不可用，不会在元数据未变时永久毒化进程。
...
`synchronous=FULL` 和 30 秒 busy timeout。缺失主库先以 0600 原子创建；既有主库先拒绝
符号链接/多硬链接，再通过已核对的文件描述符收紧为 0600，之后才捕获身份并交给 SQLx。
WAL 允许读取与一个写事务并行，但 SQLite 对同一个数据库文件仍只有一个写入者；项目因此用
进程级写门控避免自身请求反复撞上 `SQLITE_BUSY`，并用 `BEGIN IMMEDIATE` 在读改写事务
开始时就取得写入权，避免 deferred transaction 升级死锁。

遥测历史每批删除 10 000 行、批间让出 50 毫秒，并持续分批到启动时固定的超期集合追平；
审计日志每批删除 1 000 行并在批间主动让出调度。两者都让每批独立提交并累计精确删除数，
目的是缩短独占写入时段，让 Agent 上报和配置写入可以在批次之间进入。删除后的页进入 SQLite freelist，
...
- 会话令牌为随机 UUID，仅存进程内存，绝对有效期上限为 7 天；重启或改密后失效
- 会话 Cookie 是不带 `Max-Age`/`Expires` 的浏览器会话 Cookie；关闭浏览器后失效。它使用
...
既有最终目录；不会对未经认领的路径执行 chmod。bootstrap/restore 只能新建目录或认领精确
0700 的既有空目录，并写入版本独立、0600 的 `.unionc-data-directory` marker。既有非空目录
缺少 marker 时一律拒绝，当前版本不会根据旧文件名推断归属。普通启动与维护命令只接受已认领
目录，底层锁、密钥、配置和数据库代码也不再绕过前置策略重建父目录。
