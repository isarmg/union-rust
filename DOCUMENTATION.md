...
1 秒 TTL 的单航缓存，过期时并发请求只触发一个探测。连接池复用旧连接或建立新连接时还会
同时核对路径身份与 SQLite 实际打开的 main 文件句柄。公开 `/api/ready`、需要库的管理面路径
以及 Agent 配对/上报共同使用这条边界。这些检查发生在连接签出边界，无法撤回已在执行的
事务，因此绝不能把它当作支持运行中替换数据库的热切换协议。
