...
- 修改密码后由后端撤销其他会话；Web 随后主动注销调用会话，要求全部浏览器重新登录；
- 登录限流同时考虑来源 IP 与用户名；
...
- 当前常驻 `run` 收到 401（未知/失效/被替换 credential）或 403（主机生命周期撤销或
  credential/`host_id` 失配）：写 `reauth_required` 并停止投递，但当前进程继续采样到有界
  spool；一次性投递命令只入队并失败；
...
| Agent 身份收到 401/403 | 当前 `run` 停止投递并等待同实例重新配对；Web 撤销要到下次报告才被本机感知 | 只影响该实例 |
...
| 路由族 | 调用者与权限 | 已实现能力 |
|---|---|---|
| `/api/auth/*` | 浏览器；登录后使用 Cookie，会话内修改需 CSRF | JSON 登录、当前用户、注销、改密；后端改密撤销其他会话及其 SSE，Web 再注销当前会话 |
...
| 领域 | 权威实现 | 阅读重点 |
|---|---|---|
| 共享 Agent 协议 | [`protocol/src/lib.rs`](protocol/src/lib.rs) | schema v1、主机/CPU/内存/网络/磁盘/温度/GPU/capability 与 ACK DTO |
...
| Agent 控制面与数据面 | [`server/src/monitoring/http.rs`](server/src/monitoring/http.rs)、[`server/src/monitoring/model.rs`](server/src/monitoring/model.rs)、[`server/src/monitoring/store.rs`](server/src/monitoring/store.rs) | v1/v2 接入、校验、幂等、乱序、查询、撤销、重配对和保留 |
...
| Agent 运行与配置 | [`agent/src/main.rs`](agent/src/main.rs)、[`agent/src/config.rs`](agent/src/config.rs)、[`agent/src/lib.rs`](agent/src/lib.rs) | 命令选择、采样/投递 worker、jitter、停止语义和配置约束 |
| Agent 身份与可靠投递 | [`agent/src/pairing.rs`](agent/src/pairing.rs)、[`agent/src/transport.rs`](agent/src/transport.rs)、[`agent/src/spool.rs`](agent/src/spool.rs) | 可恢复配对、secret、ACK、错误分类、有界磁盘队列 |
| Agent 采集 | [`agent/src/collectors`](agent/src/collectors) | 三平台基础指标、Linux hwmon/DRM、Windows WDDM、可选 NVML |
| Windows 本机入口 | [`agent/src/bin/unionc-agent-tray.rs`](agent/src/bin/unionc-agent-tray.rs)、[`agent/src/bin/unionc-agent-maintenance.rs`](agent/src/bin/unionc-agent-maintenance.rs) | 托盘、随机回环页、UAC 固定命令、SCM 与 MSI 生命周期辅助 |
| Web 框架与请求层 | [`web/src/App.tsx`](web/src/App.tsx)、[`web/src/api.ts`](web/src/api.ts)、[`web/src/hooks.ts`](web/src/hooks.ts) | 认证状态、内存导航、懒加载、CSRF、超时、SSE 与轮询 |
| Web 业务页面 | [`web/src/views`](web/src/views) | 总览、主机/邀请/历史、公开激活、Sunshine、日志和设置 |
