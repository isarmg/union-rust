...
pub(crate) fn report_ready() -> anyhow::Result<()> {
    let Some(notify_socket) = std::env::var_os("NOTIFY_SOCKET") else {
        // Development and direct foreground runs are not supervised by systemd.
        return Ok(());
    };
    send_ready(&notify_socket)
}

fn send_ready(notify_socket: &OsStr) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn reports_ready_to_filesystem_socket() {
        let socket_path = PathBuf::from("/tmp").join(unique_name("unionc-notify"));
        let cleanup = SocketFile(socket_path.clone());
        let receiver = UnixDatagram::bind(&socket_path).unwrap();

        send_ready(socket_path.as_os_str()).unwrap();
    ...
    fn reports_ready_to_abstract_socket() {
        let abstract_name = unique_name("unionc-notify");
        let address =
        ...
        send_ready(OsStr::new(&notify_socket)).unwrap();
        let mut message = [0_u8; 32];
        let received = receiver.recv(&mut message).unwrap();
    ...
    fn reports_a_missing_notification_socket() {
        let socket_path = PathBuf::from("/tmp").join(unique_name("unionc-missing"));
