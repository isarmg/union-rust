...
test_root=$(mktemp -d "${TMPDIR:-/tmp}/unionc-server-package-builder-test.XXXXXX")
...
cleanup() {
  case "$test_root" in
    "${TMPDIR:-/tmp}"/unionc-server-package-builder-test.*)
      rm -rf -- "$test_root"
      ;;
    *)
...
mkdir -p \
  "$fixture_root/server/packaging/linux" \
  "$fixture_root/target/release" \
  "$mock_bin"
cp "$packaging_dir/build-packages.sh" \
  "$fixture_root/server/packaging/linux/build-packages.sh"
cp "$packaging_dir/../nfpm.yaml" "$fixture_root/server/packaging/nfpm.yaml"
for lifecycle_script in postinstall.sh preremove.sh; do
  printf 'package_version=%s\n' "$package_version" \
    >"$fixture_root/server/packaging/linux/$lifecycle_script"
done
printf 'UNIONC_PACKAGE_VERSION=%s\n' "$package_version" \
  >"$fixture_root/server/packaging/linux/unionc.env.example"
...
run_builder() {
  ...
  (
    ...
    SERVER_BINARY=$test_server_binary_override
    export PATH EXECUTION_MARKER NFPM_LOG NFPM_ARCH SERVER_BINARY
    sh server/packaging/linux/build-packages.sh
  ) >"$builder_output" 2>&1
}
...
fixed_payload=$fixture_root/target/release/unionc

reset_case
write_payload "$fixed_payload" 'Advanced Micro Devices X86-64' "unionc $package_version"
assert_success amd64
grep -F -- "unionc_${package_version}_amd64.deb" "$nfpm_log" >/dev/null ||
  fail 'amd64 DEB target was not requested'
grep -F -- "unionc-${package_version}.x86_64.rpm" "$nfpm_log" >/dev/null ||
  fail 'amd64 RPM target did not use the RPM architecture name'

reset_case
write_payload "$fixed_payload" AArch64 "unionc $package_version"
assert_success arm64
grep -F -- "unionc-${package_version}.aarch64.rpm" "$nfpm_log" >/dev/null ||
...
reset_case
write_payload "$fixed_payload" 'Advanced Micro Devices X86-64' 'unionc 0.3.3'
alternate_payload=$test_root/alternate-server
write_payload "$alternate_payload" 'Advanced Micro Devices X86-64' "unionc $package_version"
test_server_binary_override=$alternate_payload
if run_builder amd64; then
  fail 'SERVER_BINARY redirected validation away from the fixed nFPM payload'
fi
...
assert_output_contains 'unsupported Server Linux package architecture: riscv64'
[ ! -s "$nfpm_log" ] || fail 'unsupported architecture reached nFPM'

echo 'Server Linux package builder tests passed'
